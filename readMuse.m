function [event, eeg] = readMuse(chData)
    ...
    for i = 1:6
        ...
        %byte1 = int2bit(rawEEGData(w),8);
        %byte2 = int2bit(rawEEGData(w+1),8);
        %byte3 = int2bit(rawEEGData(w+2),8);
        % define each of the two 12 bit numbers
        eegSample(q) = bi2de([byte2(5:8) byte1]);
        eegSample(q+1) = bi2de([byte3 byte2(1:4)]);
        %eegSample(q) = bit2int([byte2(5:8) byte1]);
        %eegSample(q+1) = bit2int([byte3 byte2(1:4)]);
        % move the indexes
        q = q + 2;
        w = w + 3;
    end
